...
def scanned(request):
    ...
    try:
        ...
        return JsonResponse({
            "status":False,
            "reason":"Student with this ID Not Found!"
        }, safe=False)
...
def studentCounter(request):
    ...
    for schedule in schedules:
        startTime = int(schedule.startTime.strftime("%H"))-6
        endTime = int(schedule.endTime.strftime("%H"))-6
        students = Student.objects.filter(section = schedule.section, bach = schedule.bach, department = schedule.department)
        for student in students:
            if startTime  in range(5, 9) or endTime in range(5, 9):
                students_at_breakfast += 1
            elif startTime  in range(9, 12) or endTime in range(9, 12):
                students_at_lunch += 1
            elif startTime  in range(13, 24) or endTime in range(13, 24):
                student_at_dinner += 1
    info ={
